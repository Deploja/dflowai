...
const CandidateCard = ({
  ...
  const getInitials = () => {
    return `${candidate.first_name.charAt(0)}${candidate.surname.charAt(
      0
    )}`.toUpperCase();
  };
  ...
  return (
    <Draggable draggableId={candidate.id} index={index}>
      {(provided, snapshot) => (
        ...
        >
          <Card
            className="cursor-pointer  shadow-sm transition-shadow duration-200 ease-in-out border bg-[#6e6e6ebd] "
                ...
                <div className="flex items-center space-x-2 flex-1 min-w-0">
                  <Avatar className="h-6 w-6 flex-shrink-0">
                    <AvatarImage
                      src={
                        avatarUrl ||
                        `https://api.dicebear.com/7.x/initials/svg?seed=${candidate.first_name}${candidate.surname}`
                      }
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs truncate">{candidate.title}</div>
                  </div>
                </div>
...
const columns = [
  ...
  { id: "not-focus", title: "Not Focus", count: 0 },
  { id: "presented-prospect", title: "Presented Prospect", count: 0 },
  {
    id: "phone-coordination",
    title: "Coordination/First Call",
    count: 0,
  },
  { id: "follow-up", title: "Follow-up/Second Call", count: 0 },
  { id: "high-interest", title: "High Interest", count: 0 },
  {
    id: "upgrading-other-agreements",
    title: "Searching For Agreements",
    count: 0,
  },
  { id: "hired", title: "Hired", count: 0 },
  { id: "not-interested", title: "Not interested", count: 0 },
...
export function CandidateKanbanBoard({
  ...
  const candidatesByColumn = columns.reduce((acc, column) => {
    acc[column.id] = sortedCandidates.filter(
      (candidate) => (candidate.status || "prospect") === column.id
    );
    return acc;
  ...
  const updateCandidateStatus = async (
    ...
    try {
      ...
      console.log(
        `Successfully updated candidate ${candidateId} status to ${newStatus}`
      );
    } catch (error) {
  ...
  return (
    <div className="min-h-screen m-6 h-full">
        ...
        <div className="flex overflow-x-auto h-full">
          {columnsWithCounts.map((column) => {
            ...
            return (
              <div
                key={column.id}
                className="flex-shrink-0 w-64 h-full bg-card border border-gray-500"
              >
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <ColumnSortDropdown
                        ...
                        onSort={(field, direction) =>
                          handleSort(column.id, field, direction)
                        }
                      />
                    </div>
                  </div>
                ...
                <Droppable droppableId={column.id}>
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`p-2 min-h-[calc(100vh-200px)] max-h-[calc(100vh-200px)] overflow-y-auto ${
