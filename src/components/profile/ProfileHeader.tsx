...
import { Circle, Edit2, Check, X, Github, Linkedin, Mail, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="flex flex-col items-center min-w-[200px]">
      ...
      <div className="flex items-center gap-2 mb-4">
        <Circle className={`h-3 w-3 fill-current ${getStatusColor(activityStatus.status)}`} />
        {isEditingStatus && isOwnProfile ? (
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={onUpdateStatus} className="h-8 w-8 p-0">
          ...
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="text-xs">
              {activityStatus.status.charAt(0).toUpperCase() + activityStatus.status.slice(1)}
            </Badge>
            {isOwnProfile && (
              <Button variant="ghost" size="sm" onClick={onEditStatus} className="h-6 w-6 p-0">
