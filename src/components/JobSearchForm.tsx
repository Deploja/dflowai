...
interface JobSearchFormProps {
  ...
  onSourceChange: (source: 'all' | 'arbetsformedlingen' | 'brainwille' | 'cinode') => void;
}

export function JobSearchForm({ onJobsFound, onLoadingChange, activeSource, onSourceChange }: JobSearchFormProps) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Starting job search with params:', { searchTerm, location, occupation, source: activeSource });
      ...
      toast({
        title: "Search error",
        description: "Something went wrong during the search. Please try again later.",
        variant: "destructive"
      });
  ...
  return (
    <div className="space-y-6">
      {/* Main Search Card */}
      <Card className="bg-white dark:bg-zinc-900 border-gray-200 dark:border-gray-800 shadow-sm">
        <CardHeader className="pb-4">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Search for jobs using keywords, location, or occupation from multiple job platforms.
          </p>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Search Tabs */}
          <div className="flex space-x-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
            <button className="flex items-center space-x-2 px-4 py-2 bg-gray-900 dark:bg-gray-100 text-white dark:text-black rounded-md text-sm font-medium">
              <Search className="h-4 w-4" />
              <span>Keywords</span>
            </button>
            <button className="flex items-center space-x-2 px-4 py-2 text-gray-600 dark:text-gray-400 rounded-md text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-700">
          ...
          <form onSubmit={handleSearch} className="space-y-4">
            <div className="flex space-x-4">
              <div className="flex-1">
                <Input
                  ...
                  className="h-12 bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-base"
                />
              </div>
              <Button 
                type="submit" 
                size="lg"
                className="h-12 px-8 bg-gray-700 hover:bg-gray-800 text-white font-medium"
              >
                Search
              </Button>
              <Button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="location" className="text-gray-700 dark:text-gray-300 mb-2 block">
                  Location
                </Label>
                <Input
                  id="location"
              ...
              <div>
                <Label htmlFor="occupation" className="text-gray-700 dark:text-gray-300 mb-2 block">
                  Occupation
                </Label>
                <Input
                  id="occupation"
